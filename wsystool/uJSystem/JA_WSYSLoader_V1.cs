...
/*  
 *  This is a bastardized version of:
 *  JAudio WaveSystem Library from https://github.com/XAYRGA/libJAudio/  
    ...
    public class JA_WSYSLoader_V1
    {
        ...
        */
        ...
        public string readArchiveName(BinaryReader aafRead)
        {
            ...
            byte[] name = new byte[0x44]; // Array for the name
            ...
            aafRead.BaseStream.Seek(ofs + 0x44, SeekOrigin.Begin); // Seek 0x70 bytes, because thats the statically allocated space for the wavegroup path. 
        ...
        public JWaveSystem loadWSYS(BeBinaryReader binStream, int Base)
        {
            ...
            for (int i=0; i < WSYSGroups.Length; i++)
            {
                ...
                WSYSScenes[i] = new JWaveScene() { mOffset = (int)binStream.BaseStream.Position };
        ...
        private JWaveDescriptor loadWave(BeBinaryReader binStream,int Base)
        {
            var newWave = new JWaveDescriptor();
            newWave.mOffset = (int)binStream.BaseStream.Position;
