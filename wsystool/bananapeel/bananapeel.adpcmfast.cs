...
namespace bananapeel
{
	public static partial class ADPCMFAST
	{
        ...
        {
            return ((nib << scale) * 4) + ((sAdpcmCoefficents[coefIndex, 0] * last) + (sAdpcmCoefficents[coefIndex, 1] * penult) >> 11);
        }
        ...
        public static int PCM16TOADPCM2(short[] pcm16, byte[] adpcm2, ref int last, ref int penult, int force_coefficient = -1)
        {
            var pcmDivisor = 1;
        retrySolveFrame:
            var bestCoefficientIndex = -1;
            ...
            for (int coefIndex = 0; coefIndex < 16; coefIndex++)
            {
                ...
                for (int scale = 0; scale < 16; scale++)
                {
                    ...
                    for (int sampleIndex = 0; sampleIndex < pcm16.Length; sampleIndex++)
                    {
                        var pcmSample = pcm16[sampleIndex] / pcmDivisor;
                        var differential = (pcmSample - copyLast);
                        var diff2 = (int)Math.Round((float)differential / ((1 << (scale))*4) ); // Calculate floating point component						

                        if (diff2 > 1 || diff2 < -2)
                            break; // this scale is too fat.

                        var sampleDecoded = decodeADPCM2Sample(diff2, coefIndex, scale, copyLast, copyPenult);
                        if (isInvalidSigned16(sampleDecoded))
                            break; // scale clipped
            ...
            if (bestCoefficientIndex < 0 || bestScale < 0)
            {
                ...
                if (pcmDivisor > 10)
                    throw new Exception("Unable to solve for coefficient and scale.");
                message($"adpcm2 failed to solve coefficient, trying again divisor = {pcmDivisor} index = {bestCoefficientIndex} scl = {bestScale} bd = {best_error}");
                goto retrySolveFrame;
            }
            var nibbles = new int[16];
            if (force_coefficient > -1)
            ...
            {
                var differential = ((pcm16[i] / pcmDivisor) - last);
                var differentialSampleScaled = (int)Math.Round((float)differential / ((1 << bestScale) * 4));
                nibbles[i] = differentialSampleScaled;
                var sampleDecoded = decodeADPCM2Sample(differentialSampleScaled, bestCoefficientIndex, bestScale, last, penult);
                penult = last;
                last = sampleDecoded;
                total_error += Math.Abs(pcm16[i] - sampleDecoded);
            }

            adpcm2[0] = (byte)((bestScale << 4) | bestCoefficientIndex);
            for (var i = 0; i < 4; ++i)           
